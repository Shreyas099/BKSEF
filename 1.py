# BKSEF-selected CNN vs fixed-kernel baseline on CIFAR-10
...
def latency_ms(model, input_shape, device, warmup=20, iters=100):
    model.eval()
...
class BKSEF_CNN(nn.Module):
    """
    3-stage CNN; each stage has B blocks. For each block, select k via BKSEF (Eq. 1).
    """
    def __init__(self,
                 in_ch: int,
                 num_classes: int,
...
# Training / Evaluation
# -----------------------------
def loaders_cifar10(batch=128, workers=0, pin=False):
    ...
    tr = torchvision.datasets.CIFAR10("./data", train=True, transform=tfm_train, download=True)
    te = torchvision.datasets.CIFAR10("./data", train=False, transform=tfm_test, download=True)
...
def train_and_eval(model, tr, te, device, epochs=5, lr=0.1, label="MODEL"):
...
def main():
    ...
    # BKSEF parameters (manuscript defaults)
    ap.add_argument("--lam", type=float, nargs=3, default=[0.45, 0.45, 0.10], help="λ1 λ2 λ3")
    ...
    # Data
    tr, te = loaders_cifar10(batch=args.batch_size, workers=args.workers, pin=bool(args.pin_memory))

    # ----------------- BKSEF model -----------------
    bksef_model = BKSEF_CNN(
        in_ch=3, num_classes=10, input_hw=(32,32),
        stage_channels=[64,128,256],
        stage_blocks=[2,2,2],
        stage_strides=[1,2,2],
        k_candidates=[[3,5,7],[3,5,7],[1,3,5]],
        lam=tuple(args.lam),
        alpha=args.alpha,
        beta=args.beta,
    ...
    print("\n=== Training BKSEF-selected model ===")
    bksef_best = train_and_eval(bksef_model, tr, te, device, epochs=args.epochs, lr=args.lr, label="BKSEF")

    bksef_lat = latency_ms(bksef_model, (1,3,32,32), device, warmup=30, iters=100)
    ...
    base_lat = latency_ms(base_model, (1,3,32,32), device, warmup=30, iters=100)
    ...
    print_table(rows, headers=["Model", "Best Test Acc", "Latency (1x3x32x32)"])
